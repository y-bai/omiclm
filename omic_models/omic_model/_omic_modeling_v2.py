...
class OmicFormerConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        n_residuals_per_layer: int = 2,     # Change to 2 if we have MLP, otherwise 1. Not used for now
        ...
        **kwargs
        ...
        self.embeding_l2norm = embeding_l2norm
...
class OmicFormerPreTrainedModel(PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs):
        ...
        out, seq_emb, scrna_emb, fusion_emb = self.model(
            seq_embedding=seq_embedding,
            cell_embedding=cell_embedding, 
            **kwargs)
        
        return {"logits": out, "seq_emb": seq_emb, "scrna_emb": scrna_emb, "fusion_emb": fusion_emb}
...
class OmicFormer(nn.Module):
    ...
    def __init__(
        ...
        out_pooling_size: int = 4,
        out_pooling_mode: str = 'adaptive',   # fisrt, last, mean, weight, adaptive, max, also need sum
        n_outputs: int = 1,
    ...
    def forward(
        ...
        # cell_embedding: (batch_size, pooled_num_cells, scrna_emb_dim)

        # t_start = time.time()
        seq_emb = self.seq_input_proj(seq_embedding)    # (batch_size, seq_len, seq_emb_dim) -> (batch_size, seq_len, hidden_dim) 
        # torch.cuda.current_stream().synchronize()  
        # t_seq_emd_end = time.time()
        # print(f"seq embedding time: {t_seq_emd_end - t_start}")
        # seq_emb encoder
        seq_emb = self.seq_emb_norm(seq_emb)
        seq_emb = self.seq_encoder(seq_emb)  # seq_emb: (batch_size, seq_len, hidden_dim) 

        scrna_emb = self.scrna_input_proj(cell_embedding)   # (batch_size, pooled_num_cells, scrna_emb_dim) -> (batch_size, pooled_num_cells, hidden_dim)
        # scrna_emb encoder
        scrna_emb = self.scrna_emb_norm(scrna_emb)
        scrna_emb = self.scrna_encoder(scrna_emb)           # (batch_size, pooled_num_cells, hidden_dim)

        if self.embeding_l2norm:
            seq_emb = F.normalize(seq_emb, p=2, dim=-1)
            scrna_emb = F.normalize(scrna_emb, p=2, dim=-1)

        fusion_emb = self.fusion(seq_emb, scrna_emb)    # fusion_emb: (batch_size, seq_len, hidden_dim)

        if self.embeding_l2norm:
            fusion_emb = F.normalize(fusion_emb, p=2, dim=-1)

        out = self.out_pooling(fusion_emb)       # out: (batch_size, out_pooling_size, hidden_dim)
        out = rearrange(out, 'b n d -> b (n d)', n=out.size(1))  # concat
        out = self.out_proj(out)  # out: (batch_size, n_outputs)

        return out, seq_emb, scrna_emb, fusion_emb
