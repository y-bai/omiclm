...
class SequenceDecoder(nn.Module):
    ...
    def forward(self, x, state=None, lengths=None, l_output=None):
        ...
        if self.mode == "last":
            ...
            # This function produce the same result as the lambda function above.
            # def restrict(x):
            #     L = x.size(-2)
...
class HyenaDNAPreTrainedModel(PreTrainedModel):
        ...
        **kwargs
        ...
        if base_model is not None:
            ...
            if config is not None:
                self.model = HyenaDNAModel(**config.to_dict(), use_head=use_head, n_classes=n_classes, device=device, dtype=dtype, **kwargs)
            else:
                raise ValueError(f"Only pretrained HyenaDNAModel model or model with config is supported.")
    ...
    def forward(self, input_ids, record_id=None, row_index=None, **kwargs):
        ...
        x = self.model(input_ids, **kwargs) # the model is the backbone model
        return (record_id, row_index, x)
    ...
    def from_pretrained(cls,
        ...
        path = os.path.dirname(pretrained_model_name_or_path)
        model_name = pretrained_model_name_or_path.split('/')[-1]
        if os.path.isdir(pretrained_model_name_or_path) and download == False:
            if config is None:
                with open(os.path.join(pretrained_model_name_or_path, 'config.json'), 'r') as config_f:
        ...
        scratch_model = HyenaDNAModel(use_head=use_head, n_classes=n_classes, device=device, dtype=dtype, **config)  # the new model format
        ...
        # NOTE here the returned `scratch_model` is the instance of `HyenaDNAModel` instead of `HyenaDNAPreTrainedModel`,
        # so it will call `HyenaDNAModel.forward()` in the example of the following code:
        # >>>
        ...
        model = cls(config=PretrainedConfig(**config), base_model=scratch_model, device=device, dtype=dtype, **kwargs)
        return model
